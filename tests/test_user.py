...
def test_user_create_no_lists():
    user = User(name='Dad', birth_date=date(1980, 1, 2))
...
def test_user_create_with_lists():
    ...
    user = User(name='Dad', birth_date=date(1980, 1, 2), illnesses=illnesses_set, allergies=allergies_set, prescriptions=prescriptions_set)
    assert user.name() == 'Dad'
    assert user.birth_date() == date(1980, 1, 2)
...
def test_user_remove_illness_typical():
    user = User(name='Dad', birth_date=date(1980, 1, 2), illnesses=['illness1', 'illness2'])
    assert user.illnesses() == {'illness1', 'illness2'}
...
def test_user_remove_allergy_typical():
    user = User(name='Dad', birth_date=date(1980, 1, 2), allergies={'substance1', 'substance2'})
    assert user.allergies() == {'substance1', 'substance2'}
...
def test_user_age_typical(monkeypatch):
    ...
    user = User(name='Dad', birth_date=date(1980, 11, 7))
    assert user.age() == 44
...
def test_user_age_no_birthday_this_year(monkeypatch):
    class MockDate(date):
        ...
        def today(cls):
            return cls(2024, 12, 2)
    monkeypatch.setattr('medihelp.user.date', MockDate)
    user = User(name='Dad', birth_date=date(1980, 12, 7))
    assert user.age() == 43